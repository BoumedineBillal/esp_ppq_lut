...
if __name__ == '__main__':
    ...
    print("Running Calibration & TQT Pipeline (no LUT)...")
    from esp_ppq.quantization.optim import TrainedQuantizationThresholdPass, BiasCorrectionPass
    ...
    pipeline_pre_lut = PFL.Pipeline([
        ...
        # ==========================================  
        # TQT Optimization (Replaces QATTrainer Loop)
        # ==========================================
    ...
    print("Optimization Phase (TQT) Done! Network is now ready.")

    """
    # Save TQT optimization result
    from esp_ppq.api.export import export_ppq_graph
    export_ppq_graph(graph=graph, platform=TargetPlatform.PPL_CUDA_INT8, 
                     graph_save_to=os.path.join(QATConfig.ESPDL_OUTPUT_DIR, "Best_yolo26n.native")) 
    """

    # ==========================================
    # 5b. POST-TQT VALIDATION
    # ==========================================
    print("\n" + "=" * 60)
    print("  5b. POST-TQT VALIDATION")
    print("=" * 60)
    from trainer import QATTrainer

    print("Initializing QATTrainer for TQT accuracy check...")
    trainer = QATTrainer(graph=graph, model_meta=model_meta, device=QATConfig.DEVICE)

    print("Running mAP evaluation after TQT optimization...")
    tqt_mAP = trainer.eval()
    print(f"\n--- TQT Results ---")
    print(f"TQT mAP50-95: {tqt_mAP:.3f}")
